program ThreePCF
  ...
  integer :: k, i, j, l,m,n,d,chunk,nbins,ind,ind1,ind2,ind3
  ...
  real(kdkind) :: rmin, rmax, dr, vec_dist,rr1,rr2,rr3
...
do l=1,nbins
    do m=1,nbins
        do n=1,nbins
            i=l+(m-1)*nbins+(n-1)*nbins*nbins    
           do j=1,nmu
             do k=1,nmu   
        print*,(l-1)*nbins,l*nbins,(m-1)*nbins,m*nbins,(n-1)*nbins,n*nbins,(j-1)/odr,j/odr,(k-1)/odr,k/odr,&
        (Zddd(i,j,k)-Zddr(i,j,k)+Zdrr(i,j,k)-Zrrr(i,j,k))/Zrrr(i,j,k)
        write(11,'(16(e14.7,1x))')(l-1)*rmax/nbins,l*rmax/nbins,(m-1)*rmax/nbins,m*rmax/nbins,&
        (n-1)*rmax/nbins,n*rmax/nbins,&
        (j-1)/odr,j/odr,(k-1)/odr,k/odr,Zddd(i,j,k),Zddr(i,j,k),Zdrr(i,j,k), &
        Zrrr(i,j,k), (Zddd(i,j,k)-Zddr(i,j,k)+Zdrr(i,j,k)-Zrrr(i,j,k))/Zrrr(i,j,k), &
...
call dist(v1,v2,rr1)
call dist(my_array(:,i),v2,rr2)
call dist(my_array(:,i),v1,rr3)

if (rr1<=0.0 .or. rr1 > rmax .or. &
    rr2<=0.0 .or. rr2 > rmax .or. &
    rr3<=0.0 .or. rr3 > rmax) then
    ind=0
    goto 33
endif
    
    
ind1=floor(rr1*nbins/(rmax))+1
ind2=floor(rr2*nbins/(rmax))+1
ind3=floor(rr3*nbins/(rmax))+1
!print*,rr1,rr2,rr3, ind1,ind2,ind3
...
ind=inds(1)+(inds(2)-1)*nbins+(inds(3)-1)*nbins*nbins
!print*, rr1,rr2,rr3,ind1,ind2,ind3,ind
