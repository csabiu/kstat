program TwoPCF
  ...
  integer, parameter :: QR_R8 = selected_real_kind(8)
  integer, parameter :: QR_R4 = selected_real_kind(4)
  real(kdkind) :: ndd,ndr,nrr,nddn,ndrn,nrrn
  real(kdkind), allocatable :: dd(:,:,:),drn(:,:,:),rr(:,:,:),xi(:,:,:),mkd(:)
  real(kdkind), allocatable :: Zdd(:,:),Zdr(:,:),Zrr(:,:),crap(:,:),wgt1(:)
  real(kdkind), allocatable :: bins(:,:), v1(:),v2(:), v3(:),v4(:)
  real(kdkind) :: rmin,rmax,sig,pi,dist1,dist2,dist
  integer :: k, i, j, d,chunk,nbins,ntbin,Nresample
  character :: file1*1000, file2*1000, outfile*1000, ranfile*1000
  type(kdtree2_result),allocatable :: resultsb(:)
  integer   ::  Ndata,Nrand,nn1,nn2,sigbin,pibin,tbin,bin,wgal,wran, threads, thread
...
if(logbins) then
  if(rmin==0) print*,'Rmin = 0 [FAIL IF LOGBINS]'
  dr=(log10(rmax)-log10(rmin))/float(nbins)
  odr=1./dr
  do i=1,nbins
    bins(i,1)=10**(log10(rmin) + dr*(float(i)-1.0))
    bins(i,2)=10**(log10(rmin) + dr*i)
   ...
   nn1=kdtree2_r_count_around_point(tp=tree,idxin=i,correltime=-1,r2=2*rmin*rmin)
    ...
    dist1=sqrt(v1(1)*v1(1)+v1(2)*v1(2)+v1(3)*v1(3)) 
     ...
     do j=nn1+1,nn2
        ...
        if(logbins) then        
          sigbin=floor((log10(sig)-log10(rmin))*odr)+1
          pibin=floor((log10(pi)-log10(rmin))*odr)+1
        else
          sigbin=floor((sig-rmin)*odr)+1
          pibin=floor((pi-rmin)*odr)+1
        endif        
...
if(myid==master) then
   ...
   write(11,'(6(e14.7,1x))') bins(i,1),bins(i,2),bins(j,1),bins(j,2),(Zdd(bin,1)-2.0*Zdr(bin,1)+Zrr(bin,1))/Zrr(bin,1),sigma(bin)
   print*,(i-1)*dr,i*dr,(j-1)*dr,j*dr,av(bin),sigma(bin)

else

   write(11,'(10(e14.7,1x))')bins(i,1),bins(i,2),bins(j,1),bins(j,2),Zdd(bin,1),Zdr(bin,1),Zrr(bin,1), &
& Zdd(bin,1)/Zrr(bin,1) - 1,Zdd(bin,1)/Zdr(bin,1) - 1, (Zdd(bin,1)-2.0*Zdr(bin,1)+Zrr(bin,1))/Zrr(bin,1)
   print*,bins(i,1),bins(i,2),bins(j,1),bins(j,2),Zdd(bin,1),Zdr(bin,1),Zrr(bin,1), &
& Zdd(bin,1)/Zrr(bin,1) - 1,Zdd(bin,1)/Zdr(bin,1) - 1, (Zdd(bin,1)-2.0*Zdr(bin,1)+Zrr(bin,1))/Zrr(bin,1)
endif
...
&       nfound=nn2,nalloc=(Nrand+Ndata),results=resultsb)
    ...
    v1=my_array(1:3,i)
      ...
      do j=2,nn2
        ...
        if(dist.le.rmin) cycle
        k = ceiling(  (dist-rmin)*odr )
        ...
        theta=abs(theta/((sqrt(v3(1)**2 + v3(2)**2 + v3(3)**2)) * (sqrt(v4(1)**2 +v4(2)**2 +v4(3)**2))))
        if(theta>1.0_kdkind) cycle  
...
& nfound=nn2,nalloc=(Nrand+Ndata),results=resultsb)
    ...
    v1=my_array(1:3,i) 
      ...
      do j=2,nn2
        ...
        if(theta>1.0_kdkind) cycle
        !theta=acos(theta)
        !tbin=floor(2*theta*ntbin/pival)+1
        tbin=floor(theta*ntbin)+1
...
open(11,file=outfile,status='unknown')
if (resample) then
write(11,'(A)')'#  theta_min, theta_max, Rmin, Rmax, XI_LS, error'
else
write(11,'(A)')'#  1-mu_min,        1-mu_max,          Rmin,                   Rmax,                &
& DD (normalised),        DR (normalised),        RR (normalised),         XI_natural,            XI_Davis, &
&              XI_LS (most accurate)'
endif
print*,'#  theta_min,        theta_max,          Rmin,                   Rmax,                  &
& DD (normalised),        DR (normalised),        RR (normalised),         XI_natural,            XI_Davis, &
&              XI_LS (most accurate)'
do i=1,nbins
  do j=ntbin,1,-1
...
if (resample) then
   write(11,'(7(e14.7,1x))')1.d0-(float(j-1)/ntbin),1.d0-(float(j)/ntbin),bins(i,1),bins(i,2), &
&   (Zdd(bin,1)-2.0*Zdr(bin,1)+Zrr(bin,1))/Zrr(bin,1),sigma(bin)
else
   write(11,'(10(e14.7,1x))') 1.-(float(j)/ntbin),1.-(float(j-1)/ntbin),bins(i,1),bins(i,2),&
&   Zdd(bin,1),Zdr(bin,1),Zrr(bin,1), &
& Zdd(bin,1)/Zrr(bin,1) - 1,Zdd(bin,1)/Zdr(bin,1) - 1, (Zdd(bin,1)-2.0*Zdr(bin,1)+Zrr(bin,1))/Zrr(bin,1)
endif
   print*,1.d0-(float(j-1)/ntbin),1.d0-(float(j)/ntbin),bins(i,1),bins(i,2),Zdd(bin,1),Zdr(bin,1),Zrr(bin,1), &
& Zdd(bin,1)/Zrr(bin,1) - 1,Zdd(bin,1)/Zdr(bin,1) - 1, (Zdd(bin,1)-2.0*Zdr(bin,1)+Zrr(bin,1))/Zrr(bin,1)
...
print*,'finished DD loop in task:', myid
...
if(resample) then
if(proj) then
write(11,'(A)')'θ_min, θ_max, XI_LS (most accurate), Xi_error'
  else
write(11,'(A)')'θ_min, θ_max, XI_LS (most accurate), Xi_error'
endif
else
  if(proj) then
write(11,'(A)')'θ_min, θ_max, DD, DR, RR, XI_natural, XI_Davis, XI_LS (most accurate)'
  else
...
if(proj) then
  do i=1,nbins
    bins(i,1)=180.d0*acos(1.d0-bins(i,1)*bins(i,1)/2.d0)/pival
    bins(i,2)=180.d0*acos(1.d0-bins(i,2)*bins(i,2)/2.d0)/pival
  enddo
endif
     ...
     USE extension, ONLY : getArgument, nArguments
      ...
      do i = 1,n,2
         ...
         case ('-proj')
            if(trim(arg)=='.true.') then
              proj=.true.
              if(myid==master) print*,'Doing 2D angular stats'
            elseif(trim(arg)=='.false.') then
              proj=.false.
            else
...
31  do i=1,Ndata
    ...
    if(proj) then
      if ( wgt ) then
          if (readjk) then
            read(11,*,err=31,end=14)aux1,aux2,wgt1(i),boot(i)
          else
            read(11,*,err=31,end=14)aux1,aux2,wgt1(i)
          endif
      else
        if (readjk) then
          read(11,*,err=31,end=14)aux1,aux2,boot(i)
        else
...
implicit none
real(kdkind) :: radius,theta,phi,x,y,z
...
implicit none
if ( wgt ) then
   nrr=sum(wgt1(Ndata+1:Ndata+Nrand))*(sum(wgt1(Ndata+1:Ndata+Nrand)))
   !nrr=sum(wgt1(Ndata+1:Ndata+Nrand))*(sum(wgt1(Ndata+1:Ndata+Nrand)))-sum(wgt1(Ndata+1:Ndata+Nrand)**2)
    if(marked) then 
        ndd=sum(wgt1(1:Ndata)*mkd(:))*sum(wgt1(1:Ndata))
        ndr=sum(wgt1(1:Ndata)*mkd(:))*sqrt(nrr)
    else
        ndd=sum(wgt1(1:Ndata))*(sum(wgt1(1:Ndata)))!-sum(wgt1(1:Ndata)**2)
...
subroutine marked_samples()
  real(kind=kdkind), parameter :: r2max=600.
  ...
  do i=1,Ndata
     ...
     call kdtree2_r_nearest_around_point(tp=tree,idxin=i,correltime=-1,r2=r2max,&
          &       nfound=nn2,nalloc=(Nrand+Ndata),results=resultsb)
